classdef SPCARTR
    ...
    properties
        ...
        A % Data matrix of the objective
        tau % if delta_{k-1} <= tau * delta_k then sigma stay the same
        Sv % svds(full(A), 1)
%         maxinner_iter % for RGBB maxiter
        ...
        feps % compare to gxnorm then judge if use the RGBB
        verbose
        ...
        zeros_eta
        % following parameter is for RGBB use
%         gradnorm
%         gradnorm_decay
%         gradnorm_min
        % following parameter is used to debug
        gX
        gY
        gU
        sparse
...
%         rec_gradnorm % record gradnorm when the minGX
%         maxinner_iter_lim % the RGBB maxiter limit 
    ...
    methods
        ...
        function ret = alm_costgrad(obj, X)
            T = X + obj.U / obj.sigma;
            ret = -2 * obj.AtA_mul(X) + obj.sigma * (T - obj.l1_prox(T, obj.mu / obj.sigma));
        end
...
%                         newton_flag = 1;
                        ...
                        obj.fail_count = 0;
                        obj.epsilon = max(obj.epsilon*0.95, obj.epsilon_min); 
...
%             end
        ...
        function obj = update(obj)
            ...
            % RGBB for robust
%             if obj.iter_num > 40 && (obj.record.gX(end) > 200*(2*obj.RGBB_COUNT^3+10)*obj.minGX || obj.record.gX(end) > obj.record.gU(end)*200*(2*obj.RGBB_COUNT^3+10)) && obj.record.gX(end) > obj.n/1e5
...
%             end
            ...
            if residual >= obj.tau * obj.residual && L > -1.4 || L > 0.4       
                if obj.epsilon > 5e-5 || obj.sigma < obj.n * 5 
                    if obj.n >= 3000
                        obj.sigma = obj.sigma * 2.25;
                    elseif obj.n >= 2000 %1.75 for <2000;1.5 for >= 2000
                        obj.sigma = obj.sigma * 2.5;
                    else
                        obj.sigma = obj.sigma * 3;
                    end 
                else
            ...
            if obj.sigma > obj.n * 30 && obj.record.gX(end) > 1e-5
               if obj.n >= 3000
                   obj.sigma = obj.sigma * 0.5;
               else
                   obj.sigma = obj.sigma * 0.7;
               end
            end

            obj.residual = residual;
            obj.iter_num = obj.iter_num + 1;
%             if obj.iter_num > 10 && obj.record.gX(end) < obj.minGX  % obj.epsilon < 1e-3 && 
...
%                     obj.rec_c0 = obj.rec_c0 + 1;
%                 end

                % back situation 1
%                 if obj.rec_c > 10  %&& obj.maxtr_iter < 80 % 8 for 500-2500
%                     obj.rec_c = 0;
% %                     obj.maxinner_iter = min(obj.maxinner_iter_lim, ceil(obj.maxinner_iter * 1.2));
%                     if obj.epsilon < 5e-6
%                         obj.maxtr_iter = min(20,ceil(obj.maxtr_iter * 1.2)); % 60 1.2 for 500 - 2500
%                     elseif obj.epsilon < 5e-3
%                         obj.maxtr_iter = min(40,ceil(obj.maxtr_iter * 1.2)); % 80 1.2 for 500 - 2500
%                     else
%                         obj.maxtr_iter = min(60,ceil(obj.maxtr_iter * 1.2));
%                     end
%                     %不加判断时* 2;%(2 for 500 - 1500 1.5 for 2000 - 2500 1.1 for 3000)
%                     if obj.sigma < obj.n * 5 || obj.epsilon > 8e-4
%                         if obj.n > 2000 %1.75 for <2000;1.5 for >= 2000
%                             obj.sigma = obj.sigma * 1.45;
%                         elseif obj.n >= 1000
%                             obj.sigma = obj.sigma * 1.5;
%                         else
%                             obj.sigma = obj.sigma * 1.65;
%                         end 
%                     end
...
%                     end
%                     obj.rec_sigma = obj.sigma;
%                     obj.maxtr_iter = 150;% 150 for <2000
...
%             obj.U = zeros([obj.n obj.r]);
            ...
            obj.record.gX = [];
            obj.record.rgradnorm = [];
            obj.record.gY = [];
            obj.record.gU = [];
            obj.record.epsilon = [];
            obj.record.sigma = [];
            obj.record.sparse = [];
            obj.record.loss = [obj.objective(obj.X)];
            obj.record.time = [0];
