...
function [xoptmain, iter, time, fv, nD, sparsity, avar, time_arr, fs, gxs,totalinneriter, numrestart] = ARPG_CM(Xinitial, H, lambda, Ll, Lu, tol, maxiter, Ftol, RPMWHmaxiter, RPMWHtol)
% Min_{X \in St} -Tr(X^T A^T A X) + lambda \|X\|_1

    fhandle = @(x)f(x, H, lambda);
    gfhandle = @(x)gf(x, H, lambda);
