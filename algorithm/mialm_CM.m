function [X, Z, ret] = mialm_CM(A, manifold, f, h, opts)
...
%         kkt_error = max(kkt_X, kkt_Y);
        ...
        temp = max(max(abs(AX - Y)))/(max(max(max(abs(AX))),max(max(abs(Y))))+1);
        ret.gx_arr(iter + 1) = kkt_error +  temp;
        cstop = (kkt_error<tol &&  temp< 5.0e-7);
